...
class VehicleArchivingRepositoryIT extends AbstractSessionIT {
    ...
    void separatesActiveHistoricalAndAdministrativeVehiclePolicies() {
        ...
        Vehicle archived = archive(sessionFixtures.vehicleFor("user"), Instant.parse("2026-08-28T10:00:00Z"));
        Vehicle foreignArchived = archive(sessionFixtures.vehicleFor("admin"), Instant.parse("2026-08-29T10:00:00Z"));
    ...
    void findsArchivedVehiclesForInclusiveEventDateBoundariesAndCurrentOwner() {
        ...
        Vehicle refuelVehicle = archive(sessionFixtures.vehicleFor("user"), Instant.parse("2026-08-28T10:00:00Z"));
        sessionFixtures.refuelFor(refuelVehicle, 1_000, dateFrom);
        Vehicle repairVehicle = archive(sessionFixtures.vehicleFor("user"), Instant.parse("2026-08-28T10:01:00Z"));
        sessionFixtures.repairFor(repairVehicle, 1_100, LocalDate.of(2024, 2, 10), 100);
        Vehicle routineVehicle = archive(sessionFixtures.vehicleFor("user"), Instant.parse("2026-08-28T10:02:00Z"));
        sessionFixtures.routineServiceFor(routineVehicle, 1_200, LocalDate.of(2024, 3, 10), 100,
            2_000, LocalDate.of(2025, 3, 10));
        Vehicle inspectionVehicle = archive(sessionFixtures.vehicleFor("user"), Instant.parse("2026-08-28T10:03:00Z"));
        sessionFixtures.inspectionFor(inspectionVehicle, 1_300, LocalDate.of(2024, 4, 10), 100, dateTo);
        Vehicle insuranceVehicle = archive(sessionFixtures.vehicleFor("user"), Instant.parse("2026-08-28T10:04:00Z"));
        sessionFixtures.insuranceFor(insuranceVehicle, 1_400, dateTo, dateFrom, dateTo, 100);

        Vehicle outsidePeriod = archive(sessionFixtures.vehicleFor("user"), Instant.parse("2026-08-28T10:05:00Z"));
        sessionFixtures.refuelFor(outsidePeriod, 1_500, dateFrom.minusDays(1));
        Vehicle foreign = archive(sessionFixtures.vehicleFor("admin"), Instant.parse("2026-08-28T10:06:00Z"));
    ...
    private Vehicle archive(Vehicle vehicle, Instant archivedAt) {
