...
- [x] 3.6 The next release tag pushes both images successfully (accepted-deferred — the `--password-stdin` login change is only exercised by a tag pipeline; no release tag will be pushed until the roadmap work is complete, possibly later. Verify this on the first tag build after merge.) — 5415b4a
